...
clean_csv <- function(path,newpath=NULL,figpath=NULL,upperYieldPath=NULL,
  ...
  # #Concurvity: 
  # #             para    s(E,N)
  # # para   1.0000000 0.1516675
  # # s(E,N) 0.5705344 1.0000000
  # # #Test field 2 - many combines, not much clustering, lots of striping
  ...
  # #              para      s(E,N)
  # # para   1.00000000 0.006646651
  # # s(E,N) 0.07873227 1.000000000
  ...
  if(st_geometry_type(fieldBoundary)[1]=='MULTIPOLYGON'){
    inFieldPart <- fieldBoundary %>% st_cast('POLYGON',warn = FALSE) %>% select(Field) %>% 
      ...
      st_intersects(dat,.) %>% sapply(.,first)
  } else {
  ...
  if(any(apply(table(inFieldPart,dat$Date_Combine),1,function(x) sum(x>0))>1)){
    ...
    for(part in 1:length(unique(inFieldPart))){ #For each field part
      ...
      if(length(levels(sDat$Date_Combine))==1){
        ...
        print(paste0('Finished part ',part,' of ',length(unique(inFieldPart)),'. Single Date/Combine.'))
        next()
      }  
      ...
      # }
      # 
      # # #Generate distances DC_centroids among sDat assigned to random Date_combine groupings
      # # #Doesn't work - far too conservative
      # # genDists2 <- function(i){ #Input not used
      # #   temp <- sDat %>% mutate(Date_Combine=sample(Date_Combine)) #Randomize Date_Combine assignment
      # #   lapply(1:nrow(DC_centroids),function(x){ #Get distances from points, and take mean
      # #     temp %>% filter(Date_Combine==DC_centroids$Date_Combine[x]) %>%
      # #       st_distance(DC_centroids[x,])}) %>% unlist() %>% mean
      # # }
      # 
      # if(ncore>1){ #Generate random distances
      ...
      # hist(ranDists,xlab='Distances',xlim=range(c(ranDists,centDist))); abline(v=centDist,col='red')
      
      ##Approach using multinomial GAM - takes way too long, and gives similar results
      # tempDat <- sDat %>% mutate(Date_Combine=as.numeric(Date_Combine)-1) %>% st_drop_geometry() %>% 
      #   select(Date_Combine,E,N) %>% group_by(Date_Combine) %>% slice_sample(n=1000) %>% ungroup()
      # tempMod <- gam(c(Date_Combine~s(E),rep(list(~s(E)),nrow(DC_centroids)-2)),
      #     family=multinom(K=nrow(DC_centroids)-1),
      #     data=tempDat)
      # summary(tempMod)
      
      # if(!(centDist>quantile(ranDists,c(0.025)) & centDist<quantile(ranDists,c(0.975)))){
      if(FALSE){ #PROBLEM - these methods above don't really work
        # Amount of clustering depends on method used. A better method might be...
        # Checking concurvity b/w spatial smoother and Date_Combine parameters
        ...
        library(mgcv)
        try({
          m1 <- bam(Yield_tha ~ Date_Combine + s(E,N,bs='ts') + 0,data=sDat,cluster=cl) #No rho term
          ar1 <- acf(resid(m1),type = 'partial',plot=FALSE)$acf[1,,] #Autocorrelation term
          m1 <- bam(Yield_tha ~ Date_Combine  + s(E,N,bs='ts') + 0,data=sDat,cluster=cl,rho=ar1) #Refit with rho
  ...
  #Columns to display
  dispCols <- c('tooLarge','qFilt','bFilt','speedFilt','dSpeedFilt',
                'posFilt','vegaFilt','allFilt','yield','yieldFilt')
  if(length(unique(dat$Date_Combine))>1) dispCols <- c(dispCols,'Date_Combine')
